    ...
    def parse_file_info(self):
        ...
        for index, line_clr in enumerate(new_lines):
            ...
            if pattern:
                ...
                constructor = SubElement(current_group, 'constructor', {'name' : pattern.group(3),
            ...
            pattern = re.findall('(DW[0-9]+_?[0-9]*)[.]([a-zA-Z0-9\[\].]+)\s*=', line_clr)
            ...
            if pattern:
                ...
                for dw, fieldname in pattern:
                    if re.search('([a-zA-Z0-9]+)\[([0-9]+)\]', fieldname):
                        dword = SubElement(current_group, dw, {'arrayname' : fieldname,
                                                               'value' : value})
                    else:
                        dword = SubElement(current_group, dw, {fieldname : value})
                continue
            #-------------------------------------------------------------
...
def cpp2xml(source):
    ...
    for r,d,f in os.walk(source):
        ...
        for thing in f:
            ...
            if thing.startswith('mhw_') and re.search('g\d', thing) and 'cmd' in thing and thing.endswith('.cpp'):
                parser_list.append(CppParser(thing, r))
